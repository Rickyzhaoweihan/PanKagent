...
def runtime(source=None):
    ...
    obj.store = SimpleNamespace(
        source=lambda rid: source or {'template_id': 'functional_traces', 'parameters': {'age_min': '17'}},
...
@pytest.mark.asyncio
async def test_refresh_preserves_answer_filters_and_deduplicates():
    obj, payload = runtime()
...
@pytest.mark.asyncio
async def test_failed_refresh_has_cooldown():
    obj, payload = runtime()
...
@pytest.mark.asyncio
async def test_nonfunctional_and_running_results_are_untouched():
