...
def test_prepared_bim_answer_uses_one_stream_and_settles_actual_cost(monkeypatch, tmp_path):
    async def scenario():
        ...
        try:
            ...
            assert "INS was detected" in answer and "12.34567" in answer
            ...
            assert call['messages'][0]['content'] == prepared.body
    ...
    async def scenario():
        ...
        async with app.router.lifespan_context(app):
            async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://127.0.0.1") as client:
                ...
                assert run["evidence"]["answer_generation"]["synthesis_mode"] == "llm_formatter"
                assert run["evidence"]["answer_generation"]["streamed"] is True
                ...
                assert len([event for event in graph_events if event["payload"].get("delta")]) >= 2
    ...
    async def scenario():
        ...
        async with app.router.lifespan_context(app):
            async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://127.0.0.1") as client:
                ...
                finals = [event for event in graph_events if event["payload"].get("delta") is False]
                assert len(finals) == 1
                assert finals[0]["payload"]["answer"] == run["graph_answer"]
                if expected_ids:
