...
async def main():
 settings=replace(Settings(),state_dir=ROOT/'sessions',budget_usd=10,provider_status_url='')
 ...
 reports=[];prior={};lock=asyncio.Semaphore(manifest['concurrency']);model_times={};reservations={}
  ...
  async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app),base_url='http://127.0.0.1',timeout=180) as client:
   async def test(case):
    async with lock:
     ...
     usage={};model_elapsed=0
     for rid,event in settled_events.items():
      if rid in reserved:model_elapsed+=(datetime.fromisoformat(event['received_at'])-datetime.fromisoformat(reserved[rid]['received_at'])).total_seconds()
     ...
     print(json.dumps(report),flush=True);keyvar.reset(token)
   for group in dict.fromkeys(c['group'] for c in cases):
    ...
    await asyncio.gather(*(test(case) for case in group_cases if case['key'] not in parents))
