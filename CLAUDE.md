...
- `GLKB_URL` (default `http://localhost:8006/stream` — the local GLKB_agent SSE endpoint for literature synthesis. The service runs on **:8006** — `uvicorn service.api:app --port 8006` from `/db/usr/rickyhan/GLKB_agent`. Set this in `.env`; older docs/defaults said `:8004`, which is wrong)
...
- **GLKB API** — local `GLKB_agent` FastAPI service at `http://localhost:8006/stream` (SSE-streaming literature synthesis; the service runs on **:8006**, launched `uvicorn service.api:app --port 8006` from `/db/usr/rickyhan/GLKB_agent`; point the app at it with `GLKB_URL` in `.env`); called by `skills/glkb/scripts/glkb_client.py`. Must be running for literature to work (else `call_glkb` returns `status:"failed"` and the literature block is simply omitted). The old remote (`glkb.dcmb.med.umich.edu/api/frontend/llm_agent`) was retired — it now 301-redirects to a static site; the client guards against that (rejects redirects / non-`event-stream` responses). HIRN is fully disabled
...
- `POST /chat/message` — **Sonnet**-classified (`_classify_followup`, `claude-sonnet-4-6`) as `follow_up` (answer from already-retrieved data, no new queries) or `new_query` (full plan for review). The classifier is **biased toward `new_query`**: it only picks `follow_up` when highly confident the question is a pure clarification answerable entirely from prior shown text — any doubt re-plans and fetches fresh data.
...
- `POST /chat/start/stream` and `POST /chat/message/stream` — streaming variants of `/chat/start` and `/chat/message` that reuse the exact same handler logic via `_stream_chat_handler` (runs the handler coroutine as a shielded task, emits an NDJSON `heartbeat` every ~15s, then a terminal `result` line carrying the full `ChatResponse`, or `error`). These keep the connection alive during the **planning** step so a slow new_query plan doesn't time out at the proxy before the plan is returned (the non-stream variants had no keep-alive on planning).
...
The slowest part of a confirm is the Sonnet rigor format/reasoning agent. `main.py:run_plan_confirm` short-circuits it: it computes `_answer_cache_fingerprint(neo4j_results, rigor, complexity, use_literature)` = `sha256` over the **sorted executed query artifacts** (`neo4j_results[*].query` — uniformly the Cypher, SQL, or functional `GET` URL; error results skipped) plus `rigor`/`complexity`/`use_literature` and `CACHE_VERSION`. NL phrasing / chat context are deliberately excluded. On a hit (`session_store.get_cached_answer`) it waits `CACHE_HIT_DELAY_SECONDS` (env, default 10 — a deliberate delay so a hit doesn't return suspiciously fast; set 0 to disable) then returns the stored pipeline JSON string directly and skips `_select_pipeline` (the rest of the flow — clean → markdown-normalize → history → follow-ups — is unchanged; the hallucination check already ran when the answer was first generated). On a miss it runs the agent then `put_cached_answer`, and **spawns a background daemon thread (`main._spawn_cache_clean`) that re-cleans the cached copy's markdown with Sonnet** (`SONNET_REPAIR_MODEL` = `claude-sonnet-4-6`, higher quality than the live Haiku pass) and overwrites the row — so subsequent hits serve clean, deterministic markdown and serve-time Haiku becomes a no-op. It skips already-clean answers (`needs_llm_repair` pre-check) and de-dups concurrent cleans per fingerprint. A **startup backfill sweep** (`server.py` lifespan) re-cleans existing cached rows whose markdown still `needs_llm_repair` (sequential, ~1s-paced, idempotent). The live path stays Haiku; the markdown normalizer also re-validates LLM output and retries once if still broken (`markdown_normalized` payload gains `model` + `still_broken`). All cache I/O is best-effort (try/except → `answer_cache_error`); a cache failure never breaks a confirm. Background-clean events: `answer_cache_clean_start` / `answer_cache_cleaned`. Persisted in `logs/sessions.sqlite`, survives restarts, benefits every confirm path (chat/plan/stream/auto_confirm). Distinct from `server.py:_confirm_results` (in-memory, `plan_session_id`-keyed, 10-min, for idempotent retries). **Invalidate after a Neo4j reload** via `POST /cache/clear` or by bumping `CACHE_VERSION` + restart. Events: `answer_cache_hit` / `answer_cache_store` / `answer_cache_cleared` / `answer_cache_error`.
