...
- Include literature context for scientific interpretation, including whether enrichment or expression is cell-specific, whether a gene is a reliable marker, and what detection or absence means biologically. These are interpretation questions even when phrased as a short yes/no question. Set literature=false for an explicit graph-only request or literature opt-out. Simple entity/identifier lookups and requests to list measured values remain graph-only by default. Literature does not add graph steps or broaden their scope.
