...
ANSWER_CONTRACT += '''
Full-record fact contract (takes precedence over example-driven wording): each answer_facts ledger was computed before selecting example records. Use its per-assay counts, source/method/throughput distributions, formal GO code names and signal roles. Keep the opening conclusion as accurate as the tables. Colocalization can involve different lead variants. same_complete_lead_set=false means the complete lead sets differ, not that they cannot overlap. Report a shared recorded lead only from shared_recorded_lead_variant_ids; an empty list establishes no shared recorded lead, a nonempty list identifies the exact overlap, and null leaves overlap unknown. Use the release-scoped temporary schema comments for imported lead-signal representatives when supplied. Otherwise use Recorded variant unless the recorded evidence establishes lead/nonlead. Never overwrite an explicit stored role. Never add an unrecorded source qualifier such as GTEx-style. Do not describe all interactions as one method or throughput class unless the full distribution verifies it. Omit unsolicited assay generalizations and speculative technical explanations such as ambient RNA, dropout, aggregation artifacts or doublets unless the user requests hypotheses or source records explicitly report them. Select only relevant supported common caveats; a caveat section is optional, never an invitation to invent uncertainty. Computed donor/sample distributions are available even if individual examples are omitted. Do not say a complete search is limited or source data unavailable because only selected records are shown to you. For an aggregate question, report aggregates and omit individual donor examples unless asked.'''

from .agent_schemas.views import temporary_comment_text
ANSWER_CONTRACT += '\n' + temporary_comment_text(
    schema_module('semantic_interpretation'),
    schema_module('graph_storage')['registry']['release'])
